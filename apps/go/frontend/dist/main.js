// Bundled frontend for Wails embed (no build step required)
const App = {
  GetSettings: () => window.go.main.App.GetSettings(),
  SaveSettings: (s) => window.go.main.App.SaveSettings(s),
  PickExcel: () => window.go.main.App.PickExcel(),
  LoadSheet: (n) => window.go.main.App.LoadSheet(n),
  RefreshContacts: (p, n, s, c) => window.go.main.App.RefreshContacts(p, n, s, c),
  PreviewMessage: (t) => window.go.main.App.PreviewMessage(t),
  WhatsAppLoginStatus: () => window.go.main.App.WhatsAppLoginStatus(),
  WhatsAppPair: () => window.go.main.App.WhatsAppPair(),
  WhatsAppConnect: () => window.go.main.App.WhatsAppConnect(),
  WhatsAppLogout: () => window.go.main.App.WhatsAppLogout(),
  StartSending: (o) => window.go.main.App.StartSending(o),
  StopSending: () => window.go.main.App.StopSending(),
  ConfirmContinue: () => window.go.main.App.ConfirmContinue(),
  RecordSendResult: (n, p, s, d) => window.go.main.App.RecordSendResult(n, p, s, d),
  RecordSentRow: (r) => window.go.main.App.RecordSentRow(r),
  ExportReport: () => window.go.main.App.ExportReport(),
  WriteBackSent: (c) => window.go.main.App.WriteBackSent(c),
};
...
function updateProgress() {
  ...
  if (total === 0) headline.textContent = 'Nog geen contacten geladen';
  else if (remaining === 1) headline.textContent = '1 student klaar om uit te nodigen';
  else headline.textContent = `${remaining} studenten klaar om uit te nodigen`;
...
async function loadSettings() {
  ...
  const theme = s.appearance === 'Donker' ? 'dark' : s.appearance === 'Licht' ? 'light' : 'system';
...
async function refreshContacts() {
  ...
  const name = $('#name-col').value;
  const sent = $('#sent-col').value;
...
async function updateWAStatus() {
  ...
  $('#wa-status-text').textContent = st.loggedIn
    ? `Gekoppeld${st.phone ? ' (' + st.phone + ')' : ''}` : 'Niet gekoppeld';
...
$('#pick-excel').addEventListener('click', async () => {
  try {
    ...
    fillSelect($('#name-col'), ['(geen)', ...r.headers], r.nameCol || '(geen)');
    fillSelect($('#sent-col'), ['(geen)', ...r.headers], r.sentCol || '(geen)');
    sentCol = r.sentCol || '';
...
['phone-col', 'name-col', 'sent-col'].forEach((id) => {
  $(`#${id}`).addEventListener('change', refreshContacts);
});
$('#country-code').addEventListener('change', refreshContacts);
$('#country-code').addEventListener('input', refreshContacts);
...
$$('.theme-btn').forEach((b) => b.addEventListener('click', () => {
  ...
  $$('.theme-btn').forEach((x) => x.classList.toggle('active', x.dataset.theme === t));
}));
...
$('#wa-connect').addEventListener('click', async () => {
  try { await App.WhatsAppConnect(); await updateWAStatus(); }
  catch (e) { alert(String(e)); }
});
...
$('#start-send').addEventListener('click', async () => {
  try {
    await App.StartSending({
      ...
      confirmEach: $('#confirm-each').checked,
      skipSent: $('#skip-sent').checked,
      startIndex: resolveIndex($('#start-select').value, 0),
      endIndex: resolveIndex($('#end-select').value, contacts.length - 1),
    });
    logStarted = false;
    log('--- Start versturen ---');
...
$('#stop-send').addEventListener('click', () => { App.StopSending(); log('Stop aangevraagd…'); });
$('#continue-send').addEventListener('click', () => { App.ConfirmContinue(); $('#continue-send').classList.add('hidden'); });
...
EventsOn('send-event', async (evt) => {
  ...
  if (evt.status === 'STOPPED' || evt.status === 'COMPLETED') {
    ...
    } catch (e) { alert('Kon Excel niet bijwerken:\n' + e); }
  }
});
